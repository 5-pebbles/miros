...
static void init_once(void) { write(1, "once\n", 5); }

// A detached thread frees its own stack region at exit; hammering the rwlock
// first makes a broken self-reap (stack use after unmap, double-free) likely to
// crash.
...
int main(void) {
  // Recursive static mutex: re-locking from the owner must not deadlock.
  ...
  pthread_once(&once_control,
               init_once); // races the worker; init must run exactly once
