...
/// `AtomicU32`-backed cell for `DetachState`; values only ever come from the enum, so reads round-trip through `from_repr` unwrapped.
...
impl AtomicDetachState {
    ...
    pub fn load(&self) -> DetachState {
        DetachState::from_repr(self.0.load(Ordering::Acquire)).unwrap()
    }
